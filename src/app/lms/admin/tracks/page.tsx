...
function TracksContent() {
  ...
  // Inline cohort editing: the id whose edit row is open, and the draft being
  // edited. Seeded from the row on open; Save PUTs it via updateTrack (which
  // also accepts the dates, so a cohort can gain or lose its schedule later —
  // an end date added after the fact is exactly what feeds the certificate flow).
  ...
  const [editDraft, setEditDraft] = useState<{
    ...
    endDate: string;
    regDeadline: string;
  }>({ name: "", courseId: "", startDate: "", endDate: "", regDeadline: "" });
  ...
  const startEdit = (t: Track) => {
    ...
    setEditDraft({
      name: t.name,
      courseId: t.course_id ? String(t.course_id) : "",
      startDate: t.start_date ?? "",
      endDate: t.end_date ?? "",
      regDeadline: t.registration_deadline ?? "",
    });
  };
  ...
  const saveEdit = async (t: Track) => {
    ...
    if (editDraft.regDeadline && editDraft.startDate && editDraft.regDeadline > editDraft.startDate) {
      setError("The registration deadline can't be after the start date.");
      return;
    }
    setSavingEditId(t.id);
    setError(null);
    try {
      const res = await fetch(OWNER_API.updateTrack(t.id), {
        method: "PUT",
        headers: { "Content-Type": "application/json", Accept: "application/json", ...ownerAuthHeaders() },
        // Dates are always sent so a cleared field clears the stored value;
        // name/course only when filled (course is required, so it always is here).
        body: JSON.stringify({
          name: trimmed,
          course_id: editDraft.courseId ? Number(editDraft.courseId) : t.course_id,
          start_date: editDraft.startDate || null,
          end_date: editDraft.endDate || null,
          registration_deadline: editDraft.regDeadline || null,
        }),
      });
      if (res.status === 401 || res.status === 403) {
        router.replace("/lms/admin/login");
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        setError(json?.message || `Could not save cohort (HTTP ${res.status}).`);
        return;
      }
      if (json?.track) {
        setTracks((prev) => prev.map((r) => (r.id === t.id ? { ...r, ...json.track } : r)));
      }
      setEditingId(null);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
                ...
                <tr className="border-b border-white/5">
                  ...
                  <td className="px-5 py-3 text-right">
                    <button
                      type="button"
                      onClick={() => startEdit(t)}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-white/15 bg-white/5 px-3 py-1.5 text-xs font-semibold text-white/80 transition hover:border-white/30 hover:bg-white/10 hover:text-white"
                    >
                    ...
                    <td colSpan={5} className="px-5 py-4">
                      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => setEditDraft((d) => ({ ...d, courseId: e.target.value }))}
                        ...
                        <div className="grid grid-cols-3 gap-2 sm:col-span-2 lg:col-span-1">
                          ...
                          <div>
                            <label className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wide text-white/50">
                              Reg. deadline
                            </label>
                            <input
                              type="date"
                              value={editDraft.regDeadline}
                              onChange={(e) => setEditDraft((d) => ({ ...d, regDeadline: e.target.value }))}
                              className={inputClass}
                            />
                          </div>
                        </div>
                      ...
                      <div className="mt-3 flex items-center gap-3">
                        <button
                          ...
                          className="rounded-full bg-site-primary px-5 py-2 text-xs font-semibold text-[#fff] transition hover:brightness-110 disabled:opacity-60"
                        >
                        ...
                        <button
                          ...
                          className="rounded-full border border-white/15 px-5 py-2 text-xs font-semibold text-white/70 transition hover:bg-white/10"
                        >
                          Cancel
                        </button>
                        <span className="text-[11px] text-white/40">
                          Clear a date to unset it — setting a past end date marks the cohort as ended.
                        </span>
                      </div>
                    </td>
                  </tr>
