...
import NavbarSearch, { type NavbarSearchItem } from "./NavbarSearch";
...
type LmsNavbarProps = {
  ...
  // When provided, the navbar search becomes a live auto-complete dropdown
  // over these items (see NavbarSearch). Student portal passes one; staff and
  // agent keep the plain redirect-to-page search.
  loadSearchItems?: () => Promise<NavbarSearchItem[]>;
  logoUrl?: string | null;
};
...
export default function LmsNavbar({
  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-white/15 bg-black/30 p-4 mb-6 sm:flex-row sm:items-center sm:justify-between [html.light_&]:border-site-border [html.light_&]:bg-site-surface [html.light_&]:shadow-sm">
      ...
      {/* Search Bar: plain redirect search, or a live auto-complete dropdown
          when the layout provides an index (student portal). */}
      <NavbarSearch
        placeholder={placeholder}
        fallbackHref={searchRedirectHref}
