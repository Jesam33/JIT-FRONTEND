...
export default function PublicPageEditor() {
  ...
  // The academy's teaching category. Read-only here (displayed on the live
  // preview), it's EDITED on the Profile page's Personal details tab.
    ...
    try {
      const res = await fetch(OWNER_API.profileUpdate, {
        ...
        body: JSON.stringify({
          tagline,
          about,
          // No niche here: it's edited on the Profile page (Personal details
          // tab); omitting the key leaves the stored value untouched.
          contact,
          socials: {
  ...
  const livePreviewProfile: InstituteProfile = useMemo(
    ...
    [tagline, about, niche, coverUrl, contact, socials],
  );
